...
def generate_rms_and_spectrum_plots(
    xsecfileindex: XsecFileIndex, title, xsec_result, outdir=""
):
    ...
    fwhms = np.linspace(
        xsec_result["fwhm_min"],
        xsec_result["fwhm_max"],
        xsec_result["fwhm_nsteps"],
        endpoint=True,
    )
    ...
    if rms_min_i < rms.size:
        ax.plot(
            (fwhms[rms_min_i] / 1e9, fwhms[rms_min_i] / 1e9),
            (np.min(rms), np.max(rms)),
            linewidth=2,
            label=f"Minimum RMS {rms_min:.2e}" f"@{fwhms[rms_min_i]/1e9:1.2g} GHz",
        )
    ...
    xsec_conv, conv, width = xsec_convolve_f(
        xsec_low, fwhms[rms_min_i] / 2, run_lorentz_f, LORENTZ_CUTOFF
    )
...
def plot_fit(ax, fwhm, pressure_diff, outliers=False):
    ...
    popt, pcov, decision = do_rms_fit(
        fwhm, pressure_diff, fit_func=fit_func, outliers=outliers
    )
    ...
    ax.scatter(pressure_diff[~decision], fwhm[~decision] / 1e9, color="red")
    ax.xaxis.set_major_formatter(HectoPascalFormatter())


def plot_fwhm_vs_pressure_difference(
    fwhm, pressure_diff, title=None, ax=None, **kwargs
):
...
def scatter_plot_by_pressure_difference_per_band(
    xsecfileindex: XsecFileIndex, rmsoutput, species=None, outliers=False, ax=None
):
    ...
    bands_n = len(bands)
    for i, band in enumerate(bands):
        rms = [x for x in rmsoutput if band[0] == x["wmin"] and band[1] == x["wmax"]]
        if not len(rms):
            continue
        xsecs = rms
        plot_fwhm_vs_pressure_difference(
            *calc_fwhm_and_pressure_difference(xsecs),
            species,
            ax,
...
def scatter_plot_by_pressure_difference(
    rmsoutput, species="unkown", outliers=False, ax=None
):
    """Scatter plot of the FWHM with the lowest RMS vs. pressure difference."""
    if not rmsoutput:
        raise RuntimeError("RMS output is empty")
...
def scatter_plot_by_temperature(
    xsecfileindex: XsecFileIndex, rmsoutput, species=None, ax=None
):
    ...
    pressures = sorted(
        set(
            p[0].pressure
            for b in xsecfileindex.cluster_by_band_and_pressure()
            for p in b
        )
    )
    bands_n = len(bands)
    for i, band in enumerate(bands):
        for p in pressures:
            xsecs = [
                ...
                if band[0] == x["wmin"]
                and band[1] == x["wmax"]
                and np.abs(p - x["target_pressure"]) < 200
            ]
            if len(xsecs) >= 2:
                plot_fwhm_vs_temperature(
                    *get_fwhm_and_temperature(xsecs),
                    species,
                    ax,
                    s=50 - i / (len(bands) - 1) * 40 if bands_n > 1 else 20,
...
def plot_temperature_differences(tpressure: List[XsecFile], t0=None, fit=None, ax=None):
    if ax is None:
        ax = plt.gca()
    ...
    if fit is None:
        for xsec in tpressure[::-1]:
            ax.plot(
                np.linspace(xsec.wmin, xsec.wmax, xsec.nfreq),
...
def temperature_fit(xsec_by_pressure: List[XsecFile], output_dir, title=None, tref=230):
    ...
    if len(tpressure) < min_nt:
        logger.info(
            f"Not enough xsecs ({len(tpressure)} available for fit "
        ...
        if tref == -1:
            ...
            logger.info(
                f"Using {len(tpressure)} xsecs at {t0.temperature}K as "
...
def temperature_fit_multi(
    xsecfileindex: XsecFileIndex, tref, output_dir, title, processes=None
):
    ...
    with mp.Pool(processes=processes) as pool:
        return pool.starmap(
            temperature_fit,
            ((x, output_dir, title, tref) for band in bands for x in band),
        )
...
def plot_xsec_records(xsec_records: Tuple[XsecRecord], ax=None):
    ...
    for xr in xsec_records:
        for fmin, fmax, xsec in zip(xr.fmin, xr.fmax, xr.xsec):
            ax.plot(
                np.linspace(f2w(fmin)/100, f2w(fmax)/100, len(xsec)),
                xsec / 10000,
                label=f"{xr.species} {f2w(fmin)/100:.0f}" f"-{f2w(fmax)/100:.0f}",
            )
